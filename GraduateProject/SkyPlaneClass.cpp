...
SkyPlaneClass::SkyPlaneClass(GameObject* gameObject):Component(gameObject)
{
	m_skyPlane = 0;
	m_vertexBuffer = 0;
	m_indexBuffer = 0;
...
bool SkyPlaneClass::Initialize(ID3D11Device* device, const WCHAR* textureFilename1, const WCHAR* textureFilename2, SkyPlaneShaderClass* shader)
{
...
void SkyPlaneClass::update()
{
	...
	m_Shader->SetRenderInfo(GetTranslation(0), GetTranslation(1), GetTranslation(2), GetTranslation(3), GetBrightness());
...
ID3D11ShaderResourceView** SkyPlaneClass::GetTextureArray()
{
	return m_CloudTexture->GetTextureArray();
}
...
bool SkyPlaneClass::LoadTextures(ID3D11Device* device,const WCHAR* textureFilename1, const WCHAR* textureFilename2)
{
	...
	m_CloudTexture = new TextureArrayClass;
	if (!m_CloudTexture)
	{
		return false;
	}
	...
	result = m_CloudTexture->Initialize(device, textureFilename1, textureFilename2);
	if (!result)
	{
		return false;
	}
