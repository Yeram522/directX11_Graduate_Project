...
bool WaterShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    CreateDDSTextureFromFile(device, L"./data/res/white.dds", nullptr, &m_waterNormalTexture);


    CreateDDSTextureFromFile(device, L"./data/res/cloud002.dds", nullptr, &m_waterReflectionTexture);
    //CreateDDSTextureFromFile(device, L"./data/res/white.dds", nullptr, &m_waterRefractionTexture);
    return true;
}
...
bool WaterShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
    XMMATRIX projectionMatrix, ID3D11ShaderResourceView** textureArray, XMFLOAT3 cameraPosition)
{
    bool result;
