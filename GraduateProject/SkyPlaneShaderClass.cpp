...
SkyPlaneShaderClass::SkyPlaneShaderClass(LightClass* Light): ShaderClass(Light,"skydome")
{
	m_vertexShader = 0;
	m_pixelShader = 0;
	m_layout = 0;
...
bool SkyPlaneShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"./data/skyplane.vs", L"./data/skyplane.ps");
	if (!result)
	{
		return false;
	}
...
void SkyPlaneShaderClass::SetRenderInfo(float firstTranslationX, float firstTranslationZ, float secondTranslationX, float secondTranslationZ, float brightness)
{
	m_firstTranslationX = firstTranslationX;
	m_firstTranslationZ = firstTranslationZ;
	m_secondTranslationX = secondTranslationX;
	m_secondTranslationZ = secondTranslationZ;
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView** textures, XMFLOAT3 cameraPosition)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, textures[0], textures[1], m_firstTranslationX, m_firstTranslationZ,
		m_secondTranslationX, m_secondTranslationZ, m_brightness);
	if (!result)
	{
		return false;
	}
