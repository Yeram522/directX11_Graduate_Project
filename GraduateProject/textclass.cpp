...
bool Text::SetFps(int fps)
{
	...
	result = UpdateSentence(m_sentence1, fpsString, 20, 20, red, green, blue, transform->m_D3D->GetDeviceContext());
	if (!result)
	{
		return false;
	}
...
bool Text::SetCpu(int cpu)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, cpuString, 20, 40, 0.0f, 1.0f, 0.0f, transform->m_D3D->GetDeviceContext());
	if (!result)
	{
		return false;
	}
...
bool Text::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, 
	int screenWidth, int screenHeight, XMMATRIX baseViewMatrix)
{
	...
	result = UpdateSentence(m_sentence1, "Hello", 800, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "Goodbye", 800, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool Text::Render()
{
	...
	result = RenderSentence(transform->m_D3D->GetDeviceContext(), m_sentence1, transform->m_worldMatrix, orthoMatrix);
	if(!result)
	{
		return false;
	}
	...
	result = RenderSentence(transform->m_D3D->GetDeviceContext(), m_sentence2, transform->m_worldMatrix, orthoMatrix);
	if(!result)
	{
		return false;
	}
...
bool Text::UpdateSentence(SentenceType* sentence, const char* text, int positionX, int positionY, 
